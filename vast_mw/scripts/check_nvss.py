...
def main():
    parser = argparse.ArgumentParser(
        description="Query NVSS catalog for a number of positions",
    ...
    for name, source in zip(names, sources):
        results = vast_mw.check_nvss(source, radius=args.radius * u.arcsec)
        level = log.info if len(results) > 0 else log.warning
        level(
            f"For source at '{vast_mw.format_radec(source)}' = '{vast_mw.format_radec_decimal(source)}', found {len(results)} NVSS matches within {args.radius} arcsec"
        )
        for k, v in sorted(results.items(), key=lambda item: item[1]['separation']):
            ...
            if args.url:
                out += f"\t{vast_mw.vizier_url(catalog='VIII/65/nvss',idname='NVSS',sourcename=k)}"
            print(out)
        if args.reg:
            if len(results) > 0:
                vast_mw.create_regfile(
                    results, radius=args.reg_radius, filename=src + "_check_nvss"
                )
                log.info("Wrote ds9 region file " + f"{src}_check_nvss.reg")
...
def main():
    parser = argparse.ArgumentParser(
        description="Query VLASS Epoch 1 QL catalog for a number of positions",
    ...
    for name, source in zip(names, sources):
        results = vast_mw.check_vlass(source, radius=args.radius * u.arcsec)
        level = log.info if len(results) > 0 else log.warning
        level(
            f"For source at '{vast_mw.format_radec(source)}' = '{vast_mw.format_radec_decimal(source)}', found {len(results)} VLASS matches within {args.radius} arcsec"
        )
        for k, v in sorted(results.items(), key=lambda item: item[1]['separation']):
            ...
            if args.url:
                out += f"\t{vast_mw.vizier_url(catalog='J/ApJS/255/30/comp',idname='CompName',sourcename=k)}"
            print(out)
        if args.reg:
            vast_mw.create_regfile(
                results, radius=args.reg_radius, filename=src + "_check_vlass"
            )
            log.info("Wrote ds9 region file " + f"{src}_check_vlass.reg")
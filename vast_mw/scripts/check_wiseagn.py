...
def main():
    parser = argparse.ArgumentParser(
        description="Query WISE AGN catalog for a number of positions",
    ...
    for name, source in zip(names, sources):
        results = vast_mw.check_wiseagn(source, radius=args.radius * u.arcsec)
        level = log.info if len(results) > 0 else log.warning
        level(
            f"For source at '{vast_mw.format_radec(source)}' = '{vast_mw.format_radec_decimal(source)}', found {len(results)} WISE AGN matches within {args.radius} arcsec"
        )
        for k, v in sorted(results.items(), key=lambda item: item[1]['separation']):
            ...
            if args.url:
                out += f"\t{vast_mw.vizier_url(catalog='J/ApJS/234/23/c75cat',idname='WISEA',sourcename=k)}"
            print(out)

        if args.reg:
            if len(results) > 0:
                vast_mw.create_regfile(
                    results, radius=args.reg_radius, filename=src + "_check_wiseagn"
                )
                log.info("Wrote ds9 region file " + f"{src}_check_wiseagn.reg")